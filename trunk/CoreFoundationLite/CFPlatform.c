...
__private_extern__ const char* _CFDLLPath(void) {
    ...
    if ('\0' == cachedPath[0]) {
#if defined(DEBUG)
        const char* DLLFileName = "CoreFoundation_debug";
#elif defined(PROFILE)
        const char* DLLFileName = "CoreFoundation_profile";
#else
        const char* DLLFileName = "CoreFoundation";
#endif
    ...
    if (home == NULL) {
		...
		if (S_OK == SHGetFolderPath(NULL, CSIDL_PROFILE, NULL, SHGFP_TYPE_CURRENT, (LPWSTR) pathChars)) {
            ...
            CFRelease(str);
                    ...
                    DWORD dwChars = GetCurrentDirectoryW(MAX_PATH + 1, (LPWSTR)currDir);
                    if (dwChars > 0) {
                        UniChar* p = currDir;
			            CFIndex len = 0;
			            CFStringRef str;
