...
NSSearchPathEnumerationState NSStartSearchPathEnumeration(NSSearchPathDirectory dir, NSSearchPathDomainMask domainMask) {
	...
	domainMask = (NSSearchPathDomainMask) (domainMask & ((1 << numDomains) - 1) & ~invalidDomains);	// Just leave useful bits in there
    if ((dir != NSAllLibrariesDirectory) && 
		(dir != NSLibraryDirectory) && 
		(dir != NSUserDirectory) && 
		(dir != NSDocumentationDirectory) && 
		(domainMask & NSLocalDomainMask) && 
		(domainMask & NSSystemDomainMask))
	{
		domainMask = (NSSearchPathDomainMask)(domainMask & ~NSSystemDomainMask);	// Hack to avoid duplication
	}
...
NSSearchPathEnumerationState NSGetNextSearchPathEnumeration(NSSearchPathEnumerationState state, char *path) {
	...
	nextRoot = getenv("NEXT_ROOT");
        if (nextRoot == NULL) {
            ...
            strcpy((char*)malloc(strlen(nextRoot) + 1), nextRoot);    // Be safe...
        }
    }

    snprintf(path, PATH_MAX, "%s%s/%s", domainInfo[curDomain].needsRootPrepended ? nextRoot : "", 
		(dirInfo[curDir - 1].alternateDomainMask & (1 << curDomain)) ? domainInfo[curDomain].alternateDomainPath : domainInfo[curDomain].domainPath, 
		dirInfo[curDir - 1].dirPath);
