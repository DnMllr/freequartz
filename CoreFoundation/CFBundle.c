...
#if DEPLOYMENT_TARGET_WINDOWS
#if _MSC_VER && !DEPLOYMENT_TARGET_WINDOWSCE
#define ustrncasecmp(x, y, z) _strnicmp_l((char *)(x), (char *)(y), (z), NULL)
#elif DEPLOYMENT_TARGET_WINDOWSCE
#define ustrncasecmp(x, y, z) _strnicmp((char *)(x), (char *)(y), (z))
#else
...
__private_extern__ void *_CFBundleDLLGetSymbolByName(CFBundleRef bundle, CFStringRef symbolName) {
    ...
    if (CFStringGetCString(symbolName, buff, 1024, kCFStringEncodingWindowsLatin1)) 
		result = (void*)GetProcAddress(bundle->_hModule, buff);
#else
	if (CFStringGetCString(symbolName, buff, 1024, kCFStringEncodingWindowsLatin1)){
		WCHAR wsbuff[1024]; 
		mbstowcs(wsbuff, buff, 1023);
		result = (void*)GetProcAddressW(bundle->_hModule, wsbuff);
	}
#endif
